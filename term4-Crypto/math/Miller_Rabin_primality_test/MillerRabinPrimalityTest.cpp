...
size_t MillerRabinPrimalityTest::calculate_iters(double target_prob)
{
    ...
    double log_value = std::log(error_prob) / std::log(0.25);
    size_t iters = static_cast<size_t>(std::ceil(log_value));
    return std::max<size_t>(iters, 1);
}

bool MillerRabinPrimalityTest::perform_single_iteration(BigInt const &n)
{
