...
namespace crypto 
{
    ...
    {
        if (data.empty() || (data.size() % block_size))
        {
    ...
    Bytes AnsiX923Padding::remove(const Bytes &data, size_t block_size) const
    {
        ...
        {
            if (data[i] != 0x00) {throw std::invalid_argument("data was padded incorrectly according to ANSI X923");}
        }
