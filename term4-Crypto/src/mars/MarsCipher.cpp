...
0xab394825, 0x9ad3048f, 0xdfea32aa, 0x659473e3, 0x623f7863, 0xf3346c59, 0xab3ab685, 0x3346a90b, 0x6b56443e, 0xc6de01f8,
0x8d421fc0, 0x9b0ed10c, 0x88f1a1e9, 0x54c1f029, 0x7dead57b, 0x8d7ba426, 0x4cf5178a, 0x551a7cca, 0x1a9a5f08, 0xfcd651b9,
0x25605182, 0xe11fc6c3, 0xb6fd9676, 0x337b3027, 0xb7c8eb14, 0x9e5fd030
};
...
namespace crypto 
{
    MarsCipher::MarsCipher()
    {

    }

    void MarsCipher::setKey(Bytes const &key)
    {
        if (key.size() < 16 || key.size() > 112)
        {
            throw std::invalid_argument("key size must be in [4, 14] (32 bit words)");
        }
        m_key_size = key.size();
        KeyExpansion(key);
    }

    size_t MarsCipher::block_size() const
    {
        return 16;
    }

    void MarsCipher::KeyExpansion(Bytes const &key)
    {
        int n = (int)(m_key_size / 4);
        uint32_t T[15] = {};
        for (int i = 0; i < n; i++) 
        {
        ...
        for (size_t j = 0; j < 4; j++)
        {
            ...
            {
                T[i] = T[i] ^ rol32(T[(i - 5 + 15) % 15] ^ T[(i - 2 + 15) % 15], 3) ^ (4*i + j);
            }
            for (size_t t = 0; t < 4; t++)
            {
                for (size_t i = 0; i < 15; i++)
                {
                    T[i] = rol32(T[i]  + (S[(i - 1 + 15) % 15]) & 0x1FF, 9);
                }
            }
            for (size_t i = 0; i < 10; i++)
            {
                m_subkeys[10*j + i] = T[(4*i) % 15];
            }
        }
        ...
        {
            uint32_t j = m_subkeys[i] & 0b11;
            uint32_t w = m_subkeys[i] | 0b11;

            uint32_t M = 0; //building M mask
            for (size_t n = 2; n < 31; n++)
            {
                size_t bit = (w >> n) & 1;
                size_t left = n;
                while (left > 0 && ((w >> (left - 1)) & 1) == bit)
                    left--;

                size_t right = n;
                while (right < 31 && ((w >> (right + 1)) & 1) == bit)
                right++;

            size_t seq_len = right - left + 1;
                if (seq_len >= 10)
                {
                    M |= (1 << n);
                }
            ...
            uint32_t r = m_subkeys[i - 1] & 0b11111;
    ...
    Bytes MarsCipher::encryptBlock(Bytes const &block)
    {
        ...
        {
            B = (B ^ S0[A & 0xFF]) + S1[ror32(A, 8) & 0xFF];
            C = C + S0[ror32(A, 16) & 0xFF];
            D = D ^ S1[ror32(A, 24) & 0xFF];
            A = ror32(A, 24) + ((i == 1 || i == 5) ? B : 0) + ((i == 0 || i == 4) ? D : 0);
        ...
        {
            uint32_t R = rol32(A, 13) * m_subkeys[2*i + 5];
            R = rol32(R, 10);
            
            uint32_t M = A + m_subkeys[2*i + 4];
            M = rol32(M, (R >> 5) & 0x1F);
            
            uint32_t S_index = M & 0x1FF;
            uint32_t L = (S[S_index] ^ ror32(R, 5) ^ R);
        ...
        for (size_t i = 0; i < 8; i++)
        {
            ...
            C = C - S0[rol32(A, 8) & 0xFF];
            
            D = (D - S1[rol32(A, 16) & 0xFF]) ^ S0[rol32(A, 24) & 0xFF];
    ...
    Bytes MarsCipher::decryptBlock(Bytes const &block)
    {
        uint32_t A = block[0] | (block[1] << 8) | (block[2] << 16) | (block[3] << 24);
        uint32_t B = block[4] | (block[5] << 8) | (block[6] << 16) | (block[7] << 24);
        uint32_t C = block[8] | (block[9] << 8) | (block[10] << 16) | (block[11] << 24);
        ...
        for (size_t i = 0; i < 8; i++)
        {
            uint32_t temp = ror32(D, 24);
            D = C;
            C = B;
            B = A;
            A = temp;
            D = (D ^ S0[rol32(A, 24) & 0xFF]) + S1[rol32(A, 16) & 0xFF];
        ...
        for (size_t i = 0; i < 16; i++)
        {
            ...
            uint32_t temp = ror32(D, 13);
            D = C;
            C = B;
            B = A;
            A = temp;
            
            uint32_t R = rol32(A, 13) * m_subkeys[2*idx + 5];
            R = rol32(R, 10);
            
            uint32_t M = A + m_subkeys[2*idx + 4];
            M = rol32(M, (R >> 5) & 0x1F);
            
            uint32_t S_index = M & 0x1FF;
            uint32_t L = (S[S_index] ^ (R >> 5) ^ R);
            L = rol32(L, R & 0x1F);
        ...
        for (size_t i = 0; i < 8; i++)
        {
            ...
            A = rol32(A, 24);
            A = A - ((round == 1 || round == 5) ? B : 0) - ((round == 0 || round == 4) ? D : 0);
            
            D = D ^ S1[ror32(A, 24) & 0xFF];
            
            C = C - S0[ror32(A, 16) & 0xFF];
            
            B = (B - S1[ror32(A, 8) & 0xFF]) ^ S0[A & 0xFF];
        }
        
        // Reverse initial addition
        A -= m_subkeys[0];
        B -= m_subkeys[1];
        C -= m_subkeys[2];
        D -= m_subkeys[3];
    ...
    uint32_t MarsCipher::rol32(uint32_t x, int n)
    {
        return (x << n) | (x >> (32 - n));
    }

    uint32_t MarsCipher::ror32(uint32_t x, int n)
    {
        return (x >> n) | (x << (32 - n));
    }
}